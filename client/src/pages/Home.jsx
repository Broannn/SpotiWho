    ...
    <div style={{ minHeight: "calc(100vh - 70px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem", gap: "4rem" }}>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} style={{ textAlign: "center", maxWidth: 500 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
          ...
          {/* Always visible — anyone can play */}
          <button onClick={() => navigate("/lobby")} style={{
            ...
            boxShadow: "0 4px 24px var(--green-glow)", width: "100%", maxWidth: "280px",
          }}>Play Now</button>

          {/* Login only needed for likes mode */}
          {!user && (
            <a href="/auth/login" style={{
              display: "inline-flex", alignItems: "center", justifyContent: "center", gap: "0.7rem",
              background: "transparent", color: "var(--text-secondary)", padding: "0.75rem 2rem",
              borderRadius: "var(--radius-full)", fontSize: "0.9rem", fontWeight: 500,
              border: "1px solid var(--glass-border)", width: "100%", maxWidth: "280px",
            }}>
              ...
              Connect Spotify (for Likes mode)
            </a>
          )}
        </motion.div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
        ...
        {[
          ...
          { icon: "🏆", title: "Compete", desc: "Score based on speed" },
        ].map((f, i) => (
          <div key={i} style={{
